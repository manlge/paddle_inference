...
use crate::{
    common::{OneDimArrayInt32, TwoDimArraySize},
    ctypes::*,
};
use std::borrow::Cow;
use std::ffi::CStr;
...
impl Tensor {
    ...
    pub fn reshape(&self, shape: &[i32]) {
        unsafe { PD_TensorReshape(self.ptr, shape.len(), shape.as_ptr() as *mut _) };
    }
    ...
    pub fn data_type(&self) -> PD_DataType {
...
impl Tensor {
    pub fn copy_from_f32(&self, data: &[f32]) {
        unsafe { PD_TensorCopyFromCpuFloat(self.ptr, data.as_ptr()) };
    }

    pub fn copy_from_i64(&self, data: &[i64]) {
        unsafe { PD_TensorCopyFromCpuInt64(self.ptr, data.as_ptr()) };
    }

    pub fn copy_from_i32(&self, data: &[i32]) {
        unsafe { PD_TensorCopyFromCpuInt32(self.ptr, data.as_ptr()) };
    }

    pub fn copy_from_u8(&self, data: &[u8]) {
        unsafe { PD_TensorCopyFromCpuUint8(self.ptr, data.as_ptr()) };
    }

    pub fn copy_from_i8(&self, data: &[i8]) {
        unsafe { PD_TensorCopyFromCpuInt8(self.ptr, data.as_ptr()) };
    }
}
...
impl Tensor {
    ...
    fn check_data_type(&self, ty: PD_DataType) -> bool {
        let dt = self.data_type();
        dt == ty
    }

    fn check(&self, size: usize, ty: PD_DataType) -> bool {
...
impl Tensor {
    ...
    pub fn copy_to_f32(&self, data: &mut [f32]) -> bool {
        if self.check(data.len(), PD_DATA_FLOAT32) {
            unsafe { PD_TensorCopyToCpuFloat(self.ptr, data.as_mut_ptr()) };
            true
        } else {
            false
        }
    ...
    pub fn copy_to_i64(&self, data: &mut [i64]) -> bool {
        if self.check(data.len(), PD_DATA_INT64) {
            unsafe { PD_TensorCopyToCpuInt64(self.ptr, data.as_mut_ptr()) };
            true
        } else {
            false
        }
    ...
    pub fn copy_to_i32(&self, data: &mut [i32]) -> bool {
        if self.check(data.len(), PD_DATA_INT32) {
            unsafe { PD_TensorCopyToCpuInt32(self.ptr, data.as_mut_ptr()) };
            true
        } else {
            false
        }
    ...
    pub fn copy_to_u8(&self, data: &mut [u8]) -> bool {
        if self.check(data.len(), PD_DATA_UINT8) {
            unsafe { PD_TensorCopyToCpuUint8(self.ptr, data.as_mut_ptr()) };
            true
        } else {
            false
        }
    ...
    pub fn copy_to_i8(&self, data: &mut [i8]) -> bool {
        if self.check(data.len(), PD_DATA_UINT8) {
            unsafe { PD_TensorCopyToCpuInt8(self.ptr, data.as_mut_ptr()) };
            true
        } else {
            false
        }
...
impl Tensor {
    ...
    pub fn as_mut_slice_f32(&self, place_type: PD_PlaceType) -> Option<&mut [f32]> {
        self.check_data_type(PD_DATA_FLOAT32).then(|| {
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_mut_slice_i64(&self, place_type: PD_PlaceType) -> Option<&mut [i64]> {
        self.check_data_type(PD_DATA_INT64).then(|| {
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_mut_slice_i32(&self, place_type: PD_PlaceType) -> Option<&mut [i32]> {
        self.check_data_type(PD_DATA_INT32).then(|| {
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_mut_slice_u8(&self, place_type: PD_PlaceType) -> Option<&mut [u8]> {
        self.check_data_type(PD_DATA_UINT8).then(|| {
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_mut_slice_i8(&self, place_type: PD_PlaceType) -> Option<&mut [i8]> {
        self.check_data_type(PD_DATA_UINT8).then(|| {
...
impl Tensor {
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_slice_f32(&self) -> Option<(PD_DataType, &[f32])> {
        self.check_data_type(PD_DATA_FLOAT32).then(|| {
            let mut place_type = PD_DATA_UNK;
            let mut size = 0;
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_slice_i64(&self) -> Option<(PD_DataType, &[i64])> {
        self.check_data_type(PD_DATA_INT64).then(|| {
            let mut place_type = PD_DATA_UNK;
            let mut size = 0;
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_slice_i32(&self) -> Option<(PD_DataType, &[i32])> {
        self.check_data_type(PD_DATA_INT32).then(|| {
            let mut place_type = PD_DATA_UNK;
            let mut size = 0;
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_slice_u8(&self) -> Option<(PD_DataType, &[u8])> {
        self.check_data_type(PD_DATA_UINT8).then(|| {
            let mut place_type = PD_DATA_UNK;
            let mut size = 0;
    ...
    ///
    /// 如果底层数据类型([`DataType`])不对应则返回`None`
    pub fn as_slice_i8(&self) -> Option<(PD_DataType, &[i8])> {
        self.check_data_type(PD_DATA_UINT8).then(|| {
            let mut place_type = PD_DATA_UNK;
            let mut size = 0;
...
impl Tensor {
    pub fn set_lod(&self, lod: TwoDimArraySize) {
        unsafe { PD_TensorSetLod(self.ptr, lod.ptr) };
    }
...
impl Drop for Tensor {
    fn drop(&mut self) {
        unsafe { PD_TensorDestroy(self.ptr) };
    }
}
